              ...
              <form id="insert-form">
                <div class="card-body">
                  <div class="form-group">
                    <label for="exampleInputPassword1">BROKER NAME</label>
...
<script>
  $(document).ready(function () {
    $("#data-table").DataTable({
      "ajax":"<?=base_url('staff/Staff/get_all_Brokers')?>",
      "order":[],
      "ordering":false,
...
<script>
    $(document).on("click","#save-record-new",function(){
        ...
        form_data.append('broker_name',name);
        if(name!="")
        {
            $("#mfc-name").removeClass("is-invalid");
            $.ajax({
            url: baseUrl + "staff/Staff/insert_Brokers",
            type: "POST",
            data: form_data,
            contentType: false,
            processData: false,
...
<script>
    $(document).on("click","#record-edit-btn",function(){
       ...
       $.ajax({
        url: baseUrl + "staff/Staff/edit_Brokers",
        type: "POST",
        data:form_data,
        contentType: false,
        processData: false,
          ...
          if ((responseData != null)) 
           {
             ...
             $("#mfc-name").parent().css("margin-bottom","0");
             $("#mfc-name-err").text("");  
             $("#mfc-name").val(responseData['broker_name']);
    ...
    $(document).on("click","#update-record-btn",function(){
       ...
       var baseUrl = "<?=base_url()?>";
        ...
        if(name!="")
        {
            ...
            $.ajax({
            url: baseUrl + "staff/Staff/update_Brokers",
            type: "POST",
            data: form_data,
            contentType: false,
            processData: false,
...
<script>
    $(document).on('change','#excel-input-file',function(){
       ...
       $.ajax({
        url: baseUrl + "staff/Staff/upload_excel_to_Brokers",
        type: "POST",
        data: form_data,
        contentType: false,
        processData: false,
