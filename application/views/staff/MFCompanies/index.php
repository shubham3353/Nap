          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">MUTUAL FUND COMPANIES</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="data-table" class="table table-bordered table-striped">
                  ...
                  <tr>
                    <th>S.No</th>
                    <th>MUTUAL FUND COMPANIES NAME</th>
                    <th>ACTION</th>
                  </tr>
                  </thead>
                  <tbody>
              ...
              <form id="insert-form">
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="mfc-name">
                  </div>
                  <span id="mfc-name-err" style="color:red;font-size:15px;font-style:italic"></span>
                  <div class="form-group">
...
<script>
  $(document).ready(function () {
    $("#data-table").DataTable({
      "ajax":"<?=base_url('staff/Staff/get_all_Mutual_Fund_Companies')?>",
      "order":[],
      "ordering":false,
...
<script>
    $(document).on("click","#add-new-record",function(){
       $('#insert-form')[0].reset(); 
       $("#mfc-name").removeClass("is-invalid");
       $("#mfc-name").parent().css("margin-bottom","0");
       $("#mfc-name-err").text("");
...
<script>
    $(document).on("click","#save-record-new",function(){
        ...
        var name = $("#mfc-name").val();
        
        form_data.append('mutual_fund_name',name);
        if(name!="")
        {
            $("#mfc-name").removeClass("is-invalid");
            $.ajax({
            url: baseUrl + "staff/Staff/insert_Mutual_Fund_Companies",
            type: "POST",
            data: form_data,
            contentType: false,
            processData: false,
        ...
        {
           $("#mfc-name").addClass("is-invalid");
           $("#mfc-name").parent().css("margin-bottom","0");
           $("#mfc-name-err").text("This feild is required.*");
        }
    });
</script>
...
<script>
    $(document).on("click","#record-edit-btn",function(){
       ...
       $.ajax({
        url: baseUrl + "staff/Staff/edit_Mutual_Fund_Companies",
        type: "POST",
        data:form_data,
        contentType: false,
        processData: false,
          ...
          if ((responseData != null)) 
           {
             ...
             $("#mfc-name").parent().css("margin-bottom","0");
             $("#mfc-name-err").text("");  
             $("#mfc-name").val(responseData['mutual_fund_name']);
    ...
    $(document).on("click","#update-record-btn",function(){
       ...
       var baseUrl = "<?=base_url()?>";
        ...
        if(name!="")
        {
            ...
            $.ajax({
            url: baseUrl + "staff/Staff/update_Mutual_Fund_Companies",
            type: "POST",
            data: form_data,
            contentType: false,
            processData: false,
...
<script>
    $(document).on('change','#excel-input-file',function(){
       ...
       $.ajax({
        url: baseUrl + "staff/Staff/upload_excel_to_Mutual_Fund_Companies",
        type: "POST",
        data: form_data,
        contentType: false,
        processData: false,
