          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">CRYPTO CURRENCY</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
            ...
            <div class="modal-body">
              <form id="insert-form">
                <div class="card-body">
                  ...
                  <div class="form-group">
                    <label>TODAY PRICE</label>
                    <input type="text" class="form-control" id="feild-2"/>
                   </div>
                  <div class="form-group">
                    <label for="exampleInputPassword1">YESTERDAY PRICE</label>
...
<script>
  $(document).ready(function () {
    $("#data-table").DataTable({
      "ajax":"<?=base_url('staff/Staff/get_all_crypto_currency')?>",
      "order":[],
      "ordering":false,
...
<script>
    $(document).on("click","#save-record-new",function(){
        ...
        form_data.append('currency',data1);
        form_data.append('today_price',data2);
        form_data.append('yesterday_price',data3);
        
        $.ajax({
            url: baseUrl + "staff/Staff/insert_crypto_currency",
            type: "POST",
            data: form_data,
            contentType: false,
            processData: false,
...
<script>
    $(document).on("click","#record-edit-btn",function(){
       ...
       $.ajax({
        url: baseUrl + "staff/Staff/edit_crypto_currency",
        type: "POST",
        data:form_data,
        contentType: false,
        processData: false,
        ...
        var responseData = response['responseData'];
          if ((responseData != null)) 
           {
             $("#feild-1").val(responseData['currency']);
             $("#feild-2").val(responseData['today_price']);
             $("#feild-3").val(responseData['yesterday_price']);  
...
<script>
    $(document).on("click","#update-record-btn",function(){
       ...
       var baseUrl = "<?=base_url()?>";
        ...
        $.ajax({
            url: baseUrl + "staff/Staff/update_crypto_currency",
            type: "POST",
            data: form_data,
            contentType: false,
            processData: false,
...
<script>
    $(document).on('change','#excel-input-file',function(){
       ...
       $.ajax({
        url: baseUrl + "staff/Staff/upload_excel_to_crypto_currency",
        type: "POST",
        data: form_data,
        contentType: false,
        processData: false,
